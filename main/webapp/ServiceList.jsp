...
<html lang="en">
        	...
        	<div class="main-content">
				...
				<% } %>
                	...
                	<div class="d-flex justify-content-between align-items-center header1">
                    	...
                    	<a href="CreateService.jsp" class="btn btn-primary d-flex align-items-center">
                	...
                	<div class="table-responsive mt-1 p-3">
                		<div class="d-flex justify-content-between">
                			...
                			<div>Search: <input type="search" id="serviceSearch" placeholder="service name"> </div>
                		</div>
                    	<table class="table">
                    		<colgroup>
                    			<col width="5%">
                        	...
                        	<% Vehicle v = new Vehicle(session);
                        	   ArrayList<Service> sList = v.getServiceList();
                        	   Iterator<Service> itr = sList.iterator();
                        	   int num = 0;
							   while (itr.hasNext()) {
							   Service s = itr.next();
							   num++;
                            ...
                            <tr>
                                <td><%= num %></td>
                                <td><%= s.getDate_created() %></td>
                                <td class="service-name"><%= s.getService() %></td>
                                ...
                                <td>
                                    <div class="dropdown">
                                        <button class="btn dropdown-toggle" type="button" id="dropdownMenuButton<%=s.getId() %>" data-bs-toggle="dropdown" aria-expanded="false" style="background: #31424e; color: whitesmoke; font-size: 14px;">Action</button>
                                        <ul class="dropdown-menu m-0" aria-labelledby="dropdownMenuButton<%=s.getId()%>" style="background: #3f455a">
                                            <li><a class="dropdown-item border-bottom border-secondary pb-2" href="EditService.jsp?sid=<%=s.getId() %>"><i class="fa-solid fa-pen-to-square text-primary"></i>Edit</a></li>
                                            <li><a class="dropdown-item pt-2" onclick="confirmDelete('<%=s.getId()%>')"><i class="fa-solid fa-trash text-danger"></i>Delete</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
                ...
                <script>
					document.getElementById("serviceSearch").addEventListener("keyup", function() {
    					...
    					rows.forEach(row => {
        					let serviceName = row.querySelector(".service-name");
        					if (serviceName) {
            					let text = serviceName.textContent.toLowerCase();
    	...
    	<div class="modal-dialog modal-dialog-centered">
        	<div class="modal-content" style="background: #3d545e; color: whitesmoke;">
            	...
            	<div class="modal-body pt-3 pb-0">
                	<p>⚠️ Are you sure to delete this service permanently?</p>
            	</div>
    ...
    function confirmDelete(serviceId) {
        let deleteUrl = "delete?sid=" + serviceId; // URL to delete the mechanic
