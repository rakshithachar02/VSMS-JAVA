...
<body>
        ...
        <div class="main-content">
            ...
            <div class="container1">
                ...
                <div class="d-flex justify-content-between align-items-center header1">
                    ...
                    <a href="CreateMechanic.jsp" class="btn btn-primary d-flex align-items-center">
                        <i class="fas fa-plus me-2"></i> Create New
                    </a>
                </div>
                <% if (request.getAttribute("status") != null) {%>   
        			<h4 class="text-center bg-danger text-white mb-3 pt-3"><%= request.getAttribute("status")%></h4>
        		<%}%>
        		
                <div class="table-responsive mt-1 p-3">
                	<div class="d-flex justify-content-between">
                		<p>Show <span >10 </span> entries
                		<div>Search: <input type="search" id="mechanicSearch" placeholder="mechanic name"> </div>
                	</div>
                    <table class="table">
                        <thead>
                            <tr>
                        ...
                        <tbody id="mechanicTable">
                        	<% Vehicle v = new Vehicle(session);
                        	   ArrayList<Mechanic> mList = v.getMechanicList();
                        	   Iterator<Mechanic> itr = mList.iterator();
                        	   int num =0;
							   while (itr.hasNext()) {
							   Mechanic m = itr.next();
							   num++;
                            ...
                            <tr>
                                <td><%= num %></td>
                                <td><%= m.getDate_created() %></td>
                                <td class="mechanic-name"><%= m.getName() %></td>
                                <td><%= m.getContact() %></td>
                                <td><%= m.getEmail() %></td>
                                <td>
                                <% if(m.getStatus() == 1) { %>
                                ...
                                <td>
                                    <div class="dropdown">
                                        <button class="btn dropdown-toggle" type="button" id="dropdownMenuButton<%=m.getId() %>" data-bs-toggle="dropdown" aria-expanded="false" style="background: #31424e; color: whitesmoke; font-size: 14px;">Action</button>
                                        <ul class="dropdown-menu m-0" aria-labelledby="dropdownMenuButton<%=m.getId()%>" style="background: #3f455a">
                                            <li><a class="dropdown-item border-bottom border-secondary pb-2" href="EditMechanic.jsp?mid=<%=m.getId() %>"><i class="fa-solid fa-pen-to-square text-primary"></i>Edit</a></li>
                                            <li><a class="dropdown-item pt-2" onclick="confirmDelete('<%=m.getId()%>')"><i class="fa-solid fa-trash text-danger"></i>Delete</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr id="noResultRow" style="display: none;">
                				<td colspan="7" class="text-center text-light">No results found</td>
            				</tr>
                ...
                <script>
					document.getElementById("mechanicSearch").addEventListener("keyup", function() {
    					...
    					rows.forEach(row => {
        					let mechanicName = row.querySelector(".mechanic-name");
        					if (mechanicName) {
            					let text = mechanicName.textContent.toLowerCase();
    	...
    	<div class="modal-dialog modal-dialog-centered">
        	<div class="modal-content" style="background: #3d545e; color: whitesmoke;">
            	...
            	<div class="modal-body pt-3 pb-0">
                	<p>⚠️ Are you sure to delete this mechanic permanently?</p>
            	</div>
	...
	<script>
    	function confirmDelete(mechanicId) {
        	let deleteUrl = "delete?mid=" + mechanicId; // URL to delete the mechanic
